...
export default function RecentlyViewedSection() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 pb-10">
      <h2 className="text-lg font-bold text-snow mb-4"><Clock size={12} className="inline mr-0.5" /> 最近查看</h2>
        ...
        {recent.map((clinic: RecentClinic) => (
          ...
          >
            ...
            <p className="text-xs mb-2" style={{ color: 'rgba(0,30,29,0.5)' }}>
              <MapPin size={12} className="inline mr-0.5" /> {clinic.district}
            </p>
            {clinic.rating != null && (
              <p className="text-xs font-bold mb-2" style={{ color: '#f9bc60' }}><Star size={12} className="inline mr-0.5 fill-gold text-gold" /> {clinic.rating}</p>
            )}
